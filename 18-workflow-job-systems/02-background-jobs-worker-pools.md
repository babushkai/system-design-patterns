...
This chapter owns **job records and state transitions, worker-side receive/claim/complete protocols, broker visibility/ack interaction, database-backed claims, worker concurrency, checkpoint/restart boundaries, and graceful shutdown**. [Message Queue Architecture](../05-messaging/01-message-queues.md) owns broker durability, partitioning, retention, ordering, and generic acknowledgement internals. [Poison-Message Quarantine](../05-messaging/08-dead-letter-queues.md) owns failure quarantine and redrive. [Priority, Fairness, and Backpressure](./07-priority-fairness-backpressure.md) owns cross-class scheduling and admission. [Effect Commit Protocols](./06-retry-idempotency-compensation.md) owns retry classification, idempotent external effects, and compensation. [Leases and Recovery](./08-leases-heartbeats-recovery.md) owns lease timing, progress heartbeats, and fencing theory.
