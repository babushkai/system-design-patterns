...
This chapter owns **workflow event history, replay and deterministic command matching, orchestration/activity separation, durable timers and signals, history rollover, replay performance, worker/definition versioning, and engine evolution**. [Workflow System Fundamentals](./01-workflow-system-fundamentals.md) owns mechanism selection and common control-plane boundaries. [Effect Commit Protocols](./06-retry-idempotency-compensation.md) owns activity retries, idempotent external effects, reconciliation, and compensation. [Leases and Recovery](./08-leases-heartbeats-recovery.md) owns heartbeat/lease sizing and fencing. [Workflow Observability](./09-workflow-observability-replay.md) owns operator timelines and forensic replay. [DAG Orchestration](./05-dag-orchestration.md) owns static dependency graphs and data intervals.
