...
- **帰還不能点(point of no return)を明示的に名付ける**(レガシーが表現できない新システムの書き込みが存在し始める瞬間 — [ワークフローのエフェクトコミット](../18-workflow-job-systems/06-retry-idempotency-compensation.md)境界)。その前はロールバック=ルーティング。その後はロールバック自体がマイグレーションです。意識的に、遅めに、ソークの後にスケジュールすること。
