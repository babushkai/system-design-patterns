...
**ネットワーク分断 vs プロセス分断**: 生存しているがメッセージを処理していないプロセス（GCで停止、ディスクI/Oでブロック、ノイジーネイバーによるCPU不足）は、機能的にクラスタから分断されています。「ネットワーク障害」と「プロセス障害」の区別は、他のノードの観点からは意味がありません ── どちらも沈黙のように見えます。障害モデル、分断時の運用、フェンシング、修復、復旧境界は [06 — 障害セマンティクス・検知・復旧](./06-failure-modes.md) に統合されています。
