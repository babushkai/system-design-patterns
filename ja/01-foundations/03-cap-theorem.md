...
PACELCは1つのスコアではなく、2つの判断です。このマトリクスは左から右へ読みます：
まず分断中の振る舞いを選び、次にネットワークが正常なときに何を最適化するかを選びます。

| 分断中の振る舞い ↓ / 正常時のトレードオフ → | **EL ── 正常時の低レイテンシ** | **EC ── 正常時のより強い整合性** |
|--------------------------------------------|-------------------------------|----------------------------------|
| **PA ── 分断中も可用性を保つ** | **PA/EL**: 最速の経路、最も弱い保証。例：Cassandraの`ONE`、DynamoDBの結果整合性読み取り、Redis Cluster。古いデータを許容できるキャッシュ、セッションストア、フィード、カウンター、閲覧パスに向いています。 | **PA/EC**: 分断中は処理を継続し、正常時はより多く調整します。例：majority設定のMongoDB、選択された経路でのDynamoDB強整合性読み取り。インシデント中の古い読み取りは許容できるが、通常時の読み取りは最新にしたいアプリケーションデータに向いています。 |
| **PC ── 分断中も整合性を守る** | **PC/EL**: 実践ではまれです。通常時は高速で、分断中は危険な操作を拒否するという約束ですが、実装の詳細が現れると多くの場合PA/ELまたはPC/ECに寄ります。目標アーキテクチャではなく、警告サインとして扱うべきです。 | **PC/EC**: 最も強い保証、最も高い調整コスト。例：Spanner、CockroachDB、etcd、ZooKeeper、TiDB。金融台帳、コーディネーションサービス、在庫の権威、信頼の源泉となるデータベース、古い書き込みや競合を受け入れるよりエラーを返す方がよいワークフローに向いています。 |

本番でよく選ばれるのは、高トラフィックのユーザー向け経路には**PA/EL**、
限定的な不整合を許容できるアプリケーションデータベースには**PA/EC**、
正しさが重要な状態には**PC/EC**です。**PC/EL**は通常、安定した設計目標ではありません。
