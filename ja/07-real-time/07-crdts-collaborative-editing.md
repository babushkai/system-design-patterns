...
- **ローカルファーストの書き込みパス:** ローカルストアへ即座に適用し、操作をキューに積み、接続時に同期します(ライブチャネルは[WebSocket](/07-real-time/01-polling.md#websocket)。キューはオフライン期間を生き延びます)。UIは決してネットワークを待ちません — それが「マルチプレイヤー」の手触りのすべてです。
