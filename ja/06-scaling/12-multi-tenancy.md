...
- **オンボーディング**は[耐久プロビジョニングワークフロー](../18-workflow-job-systems/04-durable-execution-workflow-engines.md)です: テナントレコード作成 → ティアのリソース確保(スキーマ? セル配置?) → デフォルトのシード → 資格情報の発行。冪等で再開可能に。営業デモはデプロイの最中にやってくるからです。
